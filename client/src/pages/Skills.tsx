        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {categories.map((cat, index) => {
              const categorySkills = skills?.filter(s => s.category === cat.label || s.category === cat.id) || [];
              ...
              return (
                <motion.div
                  ...
                  className="bg-card border border-white/5 rounded-2xl p-8 hover:border-primary/20 transition-all duration-300 relative overflow-hidden group shadow-lg"
                >
                  <div className="relative z-10">
                    <div className="flex items-center gap-4 mb-8">
                      <div className={`p-3 rounded-xl bg-white/5 ${cat.color} group-hover:scale-110 transition-transform`}>
                        {cat.icon}
                      </div>
                    ...
                    <div className="flex flex-wrap gap-3">
                      {categorySkills.map((skill) => (
                        <Badge 
                          key={skill.id} 
                          variant="secondary" 
                          className="px-4 py-2 text-sm bg-white/5 hover:bg-primary/20 hover:text-primary transition-all border-white/5 font-mono font-medium rounded-lg"
                        >
                          {skill.name}
                        </Badge>
                      ))}
